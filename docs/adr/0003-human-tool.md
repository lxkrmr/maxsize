# ADR 0003 — shrink is a human tool
...
`shrink` was conceived as "agent-first". The idea was that an AI agent would
...
they are too large. The human runs `shrink run`. Done.

An agent telling a human "please check your screenshots and run shrink" is
strictly worse than the human just running `shrink run` themselves. The human
is already in the loop at the only moment that matters — when the screenshots
exist.

Instructing an agent to invoke shrink on the human's behalf adds a layer of
...
`shrink` is a human tool. It lives in the human's workflow, not the agent's.
